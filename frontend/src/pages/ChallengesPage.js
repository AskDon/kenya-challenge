...
export default function ChallengesPage() {
  ...
  const [selectedType, setSelectedType] = useState(user?.walker_type_id || '');
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    Promise.all([api.get('/challenges'), api.get('/walker-types')])
      .then(([ch, wt]) => { setChallenges(ch.data); setWalkerTypes(wt.data); })
      .catch(() => toast.error('Failed to load data'));
  }, []);

  const handleSelect = async () => {
    if (!selectedChallenge || !selectedType) {
      toast.error('Please select a challenge and a walker type');
      return;
    }
    setLoading(true);
    try {
      await api.post('/users/select-challenge', { challenge_id: selectedChallenge, walker_type_id: selectedType });
    ...
    <div className="container-app py-8 md:py-12" data-testid="challenges-page">
          ...
          {walkerTypes.map((wt) => (
            <button key={wt.id} onClick={() => setSelectedType(wt.id)} className="text-left w-full" data-testid={`walker-type-card-${wt.id}`}>
              <Card className={`rounded-2xl border-2 transition-all duration-200 cursor-pointer h-full ${
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-3">
                    <Badge className={`rounded-full text-xs ${selectedType === wt.id ? 'bg-orange-600 text-white' : 'bg-stone-100 text-stone-600'}`}>
                      {wt.name}
                    </Badge>
                    {selectedType === wt.id && (
                  ...
                  <p className="text-3xl font-bold text-stone-900 mt-2">${wt.cost_usd}</p>
      ...
      <div className="flex justify-center">
        <Button
          ...
          disabled={loading || !selectedChallenge || !selectedType}
